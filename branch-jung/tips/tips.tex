...
본 문서의 최초작성일은 2010년 4월 10일이며, iHELP Working Group의 관리자에 의하여 수시로 갱신되고 있습니다.
...
이 문서의 작성에 여러가지 제안을 해 주신 분들은 아래와 같습니다. 
\begin{itemize}
\item 신종화
\end{itemize}
...
\paragraph{패키지와 관련하여: }
	초기에 가장 보는 에러는 ``xxx 함수가 없습니다'' 또는 ``xxx 함수를 찾을 수 없습니다''입니다. (접수: 2013-APR-15, 분류: 패키지 관련) 
	
	대부분의 경우는 사용하고자 하는 함수가 R 기본 배포판에 포함되어 있지 않은 사용자에 의해서 제공된 특정한 패키지에서 존재하기 때문입니다.  	이런 경우에는 먼저 사용하고자 하는 함수가 어떤 패키지에 존재하는지 알아야 합니다.  그리고, 해당 패키지를 설치했을 때에는 설치된 패키지를 사용할 수 있도록 로딩하는 과정을 거쳐야 합니다.
...
이 챕터에서는 데이터 처리와 분석에 필요한 기초 프로그래밍 요소를 알아봅니다. 

\section{에러과 경고에 대한 이해}
...
\section{다양한 모형에 대한 개관과 장단점}

\subsection{회귀분석}

\subsection{일반선형모델 (GLM)}

\subsection{Mixed-effect model}

\subsection{Longitudinal data analysis}

\subsection{Survivial analysis}

\subsection{Mixture and latent class model}
