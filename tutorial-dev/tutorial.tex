...
따라서, 이 문서를 읽고 있는 독자가 문서 내용에 대한 추가, 수정, 및 제안이 있다면 이를 \href{mailto:ihelp-urquestion@lists.r-forge.r-project.org}{ihelp-urquestion@lists.r-forge.r-project.org} 주소로 이메일을 보내주신다면 감사하겠습니다. 

또한, 아래에 기재된 분들의 관심과 제안들이 없었다면 이 문서가 발전될 수 없었기에 그 감사의 말씀을 올리고 싶습니다. 
\begin{itemize}
...
아주 단순하게 말하면, 프로그램을 작성한다는 것은 이렇게 블랙박스를 만드는 것에 비유할 수 있습니다. 

그러나, 몇 개의 블랙박스가 서로 어떻게 연결 및  구성되며, 형상관리는 어떻게 할 것인지, 사용자의 편의를 고려해야 하는지, 얼마나 프로그램을 자잘하게 쪼개야 하는 등의 다양한 관점을 살리는 부분은 프로그램을 하면서 경험속에서 얻어지게 됩니다.
따라서, 여기에서는 소프트웨어의 개발을 이야기 하는 것이 아니기 때문에 단순히 프로그램이라는 것을 어떤 분석을 수행하기 위한 일련의 과정을 컴퓨터가 이해할 수 있도록 하는 것으로 한정하도록 합니다.
우리는 이러한 일련의 과정을 기록하는 것을 스크립트를 작성한다고 합니다. 
...
이러한 조건문을 이해하기 위해서 아래의 예제를 생각해 봅니다.

먼저, x 라는 숫자가 
\begin{itemize}
	\item 만약 입력받은 숫자 x가 10 보다 크다면 "x는 10 보다 큽니다" 라는 출력하고,
	\item 만약 입력받은 숫자 x가 10 보다 작거나 같다면 x가 가진 값을 출력하는 것입니다.
\end{itemize}

이를 표현하자면, 

\paragraph{논리연산}
위의 문법적 표현에서 condition 이라는 것은 어떤 주어진 조건에 대한 참과 거짓을 판단하는 부분입니다.
R은 참과 거짓을 판단하는 기능을 줍니다. 
대표적인 예로는 위에서 사용했던 is.na()라는 함수의 결과는 TRUE 또는 FALSE 로 표현되었습니다. 
이러한 표현이 바로 논리적 연산의 결과입니다.
...
여기까지는 2가지의 경우에 대한 조건적 수행을 얘기했으나, 실제적으로는 조건의 수에는 매우 많을 수도 있습니다. 
...
여기까지는 객체의 구성요소들을 뽑아내는 방법을 알았지만, 실제적으로는 이들 객체의 크기가 커진다면 내가 원하는 값이 어디에 위치하는지 알려면 매우 힘이 듭니다. 
그래서, R은 자신이 원하는 값이 어디에 위치하는지에 대한 정보를 주는 which()라는 함수를 제공합니다. 

위에서 사용한 벡터 x를 살펴봅니다. 
내가 하고자 하는 것은 108 번째라는 값이 어디에 있는가를 알고 싶습니다.
