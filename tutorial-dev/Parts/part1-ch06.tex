...
데이터 실무에서 사용된 스프레드형식의 데이터 파일을 read.table()이 실제로는 어떻게 파일로부터 읽어들여 행렬의 형식을 가지는지 살펴봅니다. 

입출력은 컴퓨터와 의사소통을 하는 실제적인 방법입니다.  
하나는 키보드와 모니터를 통하여 하는 방법이 있고, 
다른 하나는 파일로 읽고 쓰는 방법이 있습니다.

\section{키보드와 모니터}

먼저 아래와 같은 파일들을 만들어 봅니다. 

file1.txt 라는 파일은 아래와 같이 내용을 입력합니다. 
...
file2.txt 라는 파일은 아래와 같이 내용을 입력합니다. 
...
이제 이렇게 저장되어 있는 내용을 scan()이라는 함수를 이용하여 읽어봅니다.
scan()함수를 이전챕터에서는 키보드의 입력을 받기 위해 사용했었습니다 (어디였더라...).
...
file3.txt 라는 파일은 아래와 같이 내용을 입력합니다. 
...
file4.txt 라는 파일은 아래와 같이 내용을 입력합니다. 
...
데이터 입력과 출력은 통계분석의 처음과 마지막이라고 할 수 있습니다.
